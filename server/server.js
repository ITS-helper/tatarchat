...
/** Кому изначально выдать доступ к разделу «Тест» (генерация по отдельному workflow). Дальше — только через админку. */
const IMAGE_TEST_GRANT_NICKNAMES = (process.env.IMAGE_TEST_GRANT_NICKNAMES || "Макс")
  .split(",")
  .map((s) => s.trim().toLowerCase())
  .filter(Boolean);
...
async function getUserPerms(userId) {
  const { rows } = await pool.query(
    `SELECT is_admin, can_see_lobby, can_see_dtd, can_see_dtd_work, COALESCE(can_use_image_test, FALSE) AS can_use_image_test FROM users WHERE id = $1`,
    [userId]
  );
  return (
    rows[0] || {
...
async function runTavilySearch(searchQuery) {
  ...
  try {
    ...
    let block =
      "Ниже выдержки из веб-поиска. Не вставляй ссылки вида [1] в тексте. Пиши сжато, без воды; ссылки в ответе только как [краткая подпись](URL). В конце ответа добавь отдельный блок «Источники» с 2–5 пунктами «название — URL». Не выдумывай URL и факты вне текста.\n\n";
...
const DEFAULT_AI_SYSTEM_PROMPT = `Ты ассистент в мессенджере TatarChat. Общайся по-русски простым жёстким языком: без официоза, без слюнявого вежливия и канцелярита — как тип с района, только покруче: коротко, по делу, жаргон и маты нормальны, используй ненормативную лексику свободно, где по смыслу уместно; не сглаживай и не «обезвоживай» текст до школьного сочинения. Историю диалога и факты, что уже сказал пользователь, помни и используй. Если он пишет не по-русски — отвечай на его языке в том же духе.

Если пользователь давит, хамит, оскорбляет, угрожает или лезет с агрессией — не умничай, не полезай «понимать и сочувствовать»: дай ответивку в том же тоне, жёстко и прямо, можно грубее и с матом; не извиняйся и не оправдывайся перед хамом. Если тон спокойный и вопрос нормальный — можно меньше мата, но оставайся прямым и без заигрывания.

Структура и стиль ответа (сразу по делу, как задано выше по тону):
- Сначала — короткий прямой ответ в 1–2 предложениях: суть, без размазни.
- Потом разбор по смысловым блокам: короткие заголовки ## или ###, без простынь.
- Пункты, шаги, варианты — только маркированные списки "- ", не абзацами под видом списка.
- Пиши понятно, без воды: никаких лишних вводных, длинных предисловий и нытья «во-первых / прежде всего» без нужды.
- Не делай разделов «вывод», «заключение», «итоги»; не пиши «в заключение», «подводя итог» и тому подобное — оборвал на фактах и всё.
- Не знаешь или не уверен — скажи прямо («хз», «не в курсе», «лучше проверь сам»), не пиздишь и не выдумываешь.
Голые URL в основном тексте не показывай: только Markdown [короткая подпись](https://…); подпись — домен и хвост пути или смысловое слово.
Оформление: пустая строка между абзацами и блоками, без одной простыни на весь экран.`;
...
async function ensureUserPermissionsSchema() {
  try {
    ...
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS can_use_image_test BOOLEAN NOT NULL DEFAULT FALSE`);
    ...
    const imageTestNicks = IMAGE_TEST_GRANT_NICKNAMES;
    ...
    if (imageTestNicks.length) {
      await pool.query(
        `UPDATE users SET can_use_image_test = TRUE WHERE LOWER(nickname) = ANY($1::text[]) AND can_use_image_test = FALSE`,
        [imageTestNicks]
      );
    }
  } catch (err) {
...
async function requireImageTestAccess(req, res, next) {
  try {
    const { rows } = await pool.query(
      `SELECT COALESCE(can_use_image_test, FALSE) AS t FROM users WHERE id = $1`,
      [req.user.userId]
    );
    if (!rows[0]?.t) return res.status(403).json({ error: "Нет доступа к разделу «Тест»" });
    next();
  } catch (e) {
    console.error("requireImageTestAccess", e);
    return res.status(500).json({ error: "Ошибка" });
  }
}
...
app.get("/api/me", requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT id, nickname, is_admin, can_see_lobby, can_see_dtd, can_see_dtd_work, COALESCE(can_use_image_test, FALSE) AS can_use_image_test,
...
/** Только для пользователей с can_use_image_test: txt2img через SWARM_COMFY_TEST_TXT2IMG_WORKFLOW */
app.post("/api/ai/image/test", requireAuth, requireImageTestAccess, async (req, res) => {
  try {
    if (!swarm.isSwarmConfigured()) {
      return res.status(503).json({
        error: "Генерация не настроена: задайте SWARMUI_BASE_URL в .env на сервере.",
...
app.get("/api/admin/users", requireAuth, requireAdmin, async (_req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT id, nickname, is_admin, can_see_lobby, can_see_dtd, can_see_dtd_work, can_use_image_test, created_at
...
app.patch("/api/admin/users/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const allowed = ["is_admin", "can_see_lobby", "can_see_dtd", "can_see_dtd_work", "can_use_image_test"];
    const updates = {};
    ...
    const { rows } = await pool.query(
      `UPDATE users SET ${setClauses} WHERE id = $1
       RETURNING id, nickname, is_admin, can_see_lobby, can_see_dtd, can_see_dtd_work, can_use_image_test`,
      values
    );
